...
def get_states(test_board):
    ...
    for free_tile_nbr in range(free_row):
        # deepcopy curent "parent state" /////////////////////////////////

        r, c = free_row[free_tile_nbr], free_col[free_tile_nbr]

        for given_direction in direction_list:
            # make movement with the given surr_square
            vehicle, direction = test_board.car_move(given_direction, r, c)
            if vehicle and direction:
                # sla current occupation op als child ///////////////////////////
...
#
# def get_states(self):
#         copied_current_state = copy.deepcopy(board.occupation)
